...
class architecture:
	...
	def initialize_architecture(this, model_file, project_name, base_path = "{}/trace_classification/".format(HOME_PATH), trace_name = "trace_",
		...
		for cat in split_trace_sets:
			assert cat in excluded_train_labels, "Category {} found in split set but has been excluded from training in the first place!"
	...
	def train_dispatch(this, optimizer, loss_function, epochs, training_length, save_pymodel, 
		...
		for ep in range(epochs):
			...
			pass_match, pass_total, fail_match, fail_total = this.output_set_labelling(training_set)
			if save_training_log == True:
			...
			pass_match, pass_total, fail_match, fail_total = this.output_set_labelling(validation_set)
			if save_training_log == True:
	...
	def output_set_labelling(this, datapoint_set):
		...
		return pass_match, pass_total, fail_match, fail_total
	...
	def process_trace(this, trace_path, encoding_size):
		...
		for line in f:
			...
			if len(line_split) > 1:
				datapoint.append({'trace_func': this.str_to_tensor(line_split[0].split(' '), 0), 'trace_ret': this.str_to_tensor(line_split[1].split(' '), encoding_size), 'trace_arg': this.str_to_tensor(line_split[2].split(' '), encoding_size)})
